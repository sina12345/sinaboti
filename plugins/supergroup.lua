...
local function lock_group_khanevade(msg, data, target)
  ...
  if group_khanevade_lock == 'yes' then
    return 'khanevade setting is already locked'
  else
    data[tostring(target)]['settings']['khanevade'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_khanevade(msg, data, target)
  ...
  local group_khanevade_lock = data[tostring(target)]['settings']['khanevade']
  if group_khanevade_lock == 'no' then
    return 'khanevade setting is not locked'
  else
    data[tostring(target)]['settings']['khanevade'] = 'no'
    save_data(_config.moderation.data, data)
...
if matches[1]:lower() == 'uexpiretime'
 and not matches[3] then
  	local hash = 'usecommands:'..msg.from.id..':'..msg.to.id
  	 redis:incr(hash)
  	  expired = 'Unlimited' 
  	  local target = msg.to.id    
  	  savelog(msg.to.id, name_log.." 
	  ["..msg.from.id.."] "has changed group expire time to [unlimited]")  
  	 ...
  	 if tonumber(matches[2]) < 95 or tonumber(matches[2]) > 96 then
  	  ...
  	  if tonumber(matches[4]) < 01 or tonumber(matches[4]) > 31 then
  	   return "سومین match باید بین 01 تا 31 باشد"
  	     ...
  	     local target = msg.to.id 
  	   savelog(msg.to.id,name_log.."
["..msg.from.id.."] has changed group expire 
time to 
