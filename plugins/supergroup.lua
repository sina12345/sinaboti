...
local function lock_group_userbot(msg, data, target)
  ...
  if group_userbot_lock == 'yes' then
    return 'userbot is already locked'
  else
    data[tostring(target)]['settings']['lock_userbot'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_userbot(msg, data, target)
  ...
  local group_userbot_lock = data[tostring(target)]['settings']['lock_userbot']
  if group_userbot_lock == 'no' then
    return 'userbot is not locked'
  else
    data[tostring(target)]['settings']['lock_userbot'] = 'no'
    save_data(_config.moderation.data, data)
...
local function lock_group_msgbot(msg, data, target)
  ...
  if group_msgbot_lock == 'yes' then
    return 'msgbot is already locked'
  else
    data[tostring(target)]['settings']['lock_msgbot'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_msgbot(msg, data, target)
  ...
  local group_msgbot_lock = data[tostring(target)]['settings']['lock_msgbot']
  if group_msgbot_lock == 'no' then
    return 'msgbot is not locked'
  else
    data[tostring(target)]['settings']['lock_msgbot'] = 'no'
    save_data(_config.moderation.data, data)
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_userbot'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_magbot'] then
			data[tostring(target)]['settings']['lock_msgbot'] = 'no'
		end
	end
	if data[tostring(target)]['settings'] then
  ...
  local text = "تنظیمات عمومی⚙\nℹ️پاک کردن خودکار تبلیغات: "..settings.lock_link.."\nℹ️پاک کردن خودکار لینک ها: "..settings.lock_ads.."\nℹ️پاک کردن خودکار شماره تلفن: "..settings.lock_contacts.."\nℹ️پاک کردن خودکار کلمات عربی: "..settings.lock_arabic.."\nℹ️حذف خودکار عضو جدید: "..settings.lock_member.."\nℹ️پاک کردن خودکار  متن شکلکی: "..settings.lock_rtl.."\nℹ️پاک کردن خودکار پیام ورود و خروج: "..settings.lock_tgservice.."\n\nتنظیمات بیشتر⚙\nℹ️پاک کردن خودکار استیکر: "..settings.lock_sticker.."\nℹ️پاک کردن خودکار هشتگ: "..settings.lock_tag.."\nℹ️پاک کردن خودکار شکلک: "..settings.lock_emogi.."\nℹ️پاک کردن خودکار انگلیسی: "..settings.lock_english.."\nℹ️پاک کردن خودکار پیام فروراد شده: "..settings.lock_fwd.."\nℹ️پاک کردن خودکار پیام رپلی شده: "..settings.lock_reply.."\nℹ️پاک کردن ورود اعضای جدید: "..settings.lock_join.."\nℹ️پاک کردن خودکار یوزرنیم: "..settings.lock_username.."\nℹ️پاک کردن خودکار رسانه ها: "..settings.lock_media.."\nℹ️پاک کردن خودکار فحش: "..settings.lock_fosh.."\nℹ️خروج از گروه=بن: "..settings.lock_leave.."\nℹ️حذف ورود ربات ها: "..bots_protection.."\nℹ️پاک کردن خودکار کلمات اوپراتور: "..settings.operator.."\nℹ️پاک کردن خودکار ویس: "..settings.lock_voice.."\nℹ️پاک کردن خودکار تصاویر: "..settings.lock_photos.."\nℹ️پاک کردن خودکار ویدئو: "..settings.lock_video.."\n\nسوییچ های آسان و سریع⚙\n|>سوییچ روی حالت اتحاد : "..settings.etehad.."\n|>سوییچ روی حالت نرمال: "..settings.normal.."\n|>سوییچ روی حالت خانواده: "..settings.khanevade.."\n\nتنظیمات مربوط به گروه⚙\n|>حساسیت به اسپم  : "..NUM_MSG_MAX.."\n|>گروه روی خصوصی  : "..settings.public.."\n|>نوع گروه  : "..gp_type.."\n|>ققل همه دستورات : "..settings.all.."\n|>سخت گیری تنظیمات : "..settings.strict
  return text
end
