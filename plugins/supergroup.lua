...
local function unlock_group_easy(msg, data, target)
  ...
  local group_easy_lock = data[tostring(target)]['settings']['lock_easy']
  if group_easy_lock == '❌' then
    return 'easy group is trafic no'
  else
    data[tostring(target)]['settings']['lock_easy'] = '❌'
    save_data(_config.moderation.data, data)
    return 'easy group is trafic no'
...
local function lock_group_heavy(msg, data, target)
  ...
  if group_easy_lock == '✅' then
    return 'heavy group is trafic yes'
  else
...
local function unlock_group_heavy(msg, data, target)
  ...
  if group_heavy_lock == '❌' then
    return 'heavy group is trafic no'
  else
    data[tostring(target)]['settings']['lock_heavy'] = '❌'
    save_data(_config.moderation.data, data)
    return 'heavy group is trafic no'
...
target),
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_etehad(msg, data, target), etehad
			...
			if matches[2] == 'easy' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked easy posting")
				return lock_group_easy(msg, data, target)
			end
           if matches[2] == 'heavy' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked heavy")
   ...
   unlock_group_video(msg, data, target),
		...
		unlock_group_operator(msg, data, target),
      	...
      	return unlock_group_etehad(msg, data, target), detehad
			...
			if matches[2] == 'heavy' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked heavy posting")
  ...
  J          if matches[2] == 'easy' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked easy")
